...
注意：使用`ssh`协议时远程仓库的命名格式，`git@[版本库域名]:[用户名前缀]/[版本库名]`
...
# fix 是跟踪分支，与origin/fix建立跟踪关系，且有1个提交领先与远程分支
```
...
本节介绍如何创建和使用远程分支、远程跟踪分支和跟踪分支。先介绍本地仓库与远程仓库间进行分支交互的方法，分别是从远程仓库拉取分支、推送分支到远程仓库。
...
一个团队项目，远程仓库会被多人不定期更新，本地仓库的远程跟踪分支可能滞后于远程仓库，需要手动将远程仓库中最新的分支数据拉取到本地。这也使用 Git 进行多人协作时需要额外付出的成本。

Git 提供两种从远程仓库拉取分支的方法

**抓取 Fetch**

使用`git fetch`指令，从远程仓库抓取**所有分支**或指定分支到本地仓库。拉取操作将更新（或创建）本地仓库中的远程跟踪分支，并不会与本地分支或跟踪分支进行合并，因此该操作不会影响当前工作目录中的文件。**是安全的**
...
因为要进行分支的合并操作，因此需要告诉 Git 远程跟踪分支要与哪个分支进行合并，而这就是**跟踪分支**的作用。上节介绍的跟踪分支就是为了为此而生的。

```bash
# 拉取远程仓库
$ git pull [远程仓库] [分支]
# 示例
# 从origin指向的远程仓库拉取master分支，并与本地master分支关联
...
在多人协作的场景中，当工作完成后需要将其推送到远程仓库，便于他人拉取最新的版本信息。在进行分支推送时我们一般会遇到以下两种种情况。
...
1. 在远程仓库创建`fea_login`远程分支
2. 将本地`fea_login`分支的提交历史推送到远程分支
3. 将远程仓库的`fea_login`分支抓取到本地并创建`origin/fea_login`的远程跟踪分支
...
在这里我们就可以在`dev`分支上进行代码的创建、修改、提交等操作
...
个人的开发工作完成后，需要把最新的提交推送到远程仓库中。项目负责人会根据版本计划将`dev`分支合并到`master`分支中，进行上线前的测试工作。

```bash
# 第一步：从远程仓库拉取最新dev分支的最新提交
$ git pull
# 如果出现合并冲突的情况，参照分支一文进行冲突的处理
# 第二步：将dev分支推动到远程仓库
$ git push
```
