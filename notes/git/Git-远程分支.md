...
上文重点介绍了Git分支，包括提交、提交树、分支的概念和作用，如何创建分支、切换分支、合并分支以及删除分支。并结合`VS Code`开发工具，介绍在本地Git仓库使用分支的操作方法和注意事项。

**提交**。是Git进行版本控制的基本单位，可以理解为每个提交是一个版本的快照。

**提交树**。提交通过`parent`属性建立与父提交的联系组成一条提交树。通过这种关联可以反向遍历提交历史。

**分支**。可以理解为一个动态的指针，其总是指向某条提交树中最后的提交。使用分支可以方便的切换工作场景（如：功能开发、代码重构、Bug修复…）而不用过多的考虑不同工作场景下代码的版本。
...
远程分支以及其指向的提交树是存储在远程仓库中的，不可能每次访问远程分支都与远程仓库通信来读取数据，Git 把远程仓库中的分支及其指向的提交树下载到本地仓库中，只有当需要更新数据时才与远程仓库进行通信。下载到本地的远程分支称为**远程跟踪分支**。
