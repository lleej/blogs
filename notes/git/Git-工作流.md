...
| 修复分支 | 临时分支 | `fix_`     | 对发布版本进行修复的分支。当发布版本发现`Bug`后，从主干分支创建<br />修复分支对代码进行修复。修复完成后删除该分支 |
| 发布分支 | 临时分支 | `release_` | 新版本发布预处理分支。在发布节点到来前，从开发分支创建发布分支<br />对新版本进行测试和`Bug`修复处理 |
| 开发分支 | 长期分支 | `dev`      | 集成开发分支。特性开发工作是基于开发分支而不是主干分支进行的，<br />它通常会领先于主干分支 |
